...
gpu::ReductionHelper::ReductionHelper(const ReductionDescriptor &dataDesc, const gpu::Context &gpuContext)
{
    ...
    assert((dataDesc.data.size / sizeof(float)) / dataDesc.unitSize % dataDesc.workgroupSize == 0 &&
    ...
    for(size_t i = 0; i < numStages; ++i) {
        ...
        if(!isLastStage) {
            m_partialSums.emplace_back(gpuContext.makeEmptyBuffer(partialSumUnits * sizeof(float) * dataDesc.unitSize));
        }
        else {
